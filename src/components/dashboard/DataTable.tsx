...
export const DataTable: React.FC<DataTableProps> = ({
  ...
  return <Card className="bg-gradient-to-br from-white via-slate-50/20 to-white border-0 shadow-2xl backdrop-blur-sm">
      <CardHeader className="pb-6 bg-gradient-to-r from-slate-50/80 via-white to-slate-50/80 rounded-t-xl">
        ...
        <div className="flex gap-4 items-center">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-white/80 backdrop-blur-sm border-slate-300 shadow-sm" />
          </div>
      ...
      <CardContent className="p-8">
        {type === 'monthly' ? renderMonthlyTable() : type === 'yoy-analysis' ? renderYoYTable() : renderStandardTable()}
        
        {/* Enhanced Pagination for YoY table */}
        {type === 'yoy-analysis' && <div className="flex items-center justify-between mt-8 p-4 bg-gradient-to-r from-slate-50/80 via-white to-slate-50/80 rounded-xl border border-slate-200/50 shadow-sm">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="px-4 shadow-sm hover:shadow-md transition-all duration-300">
              ...
              const page = i + 1;
              return <Button key={page} variant={currentPage === page ? "default" : "outline"} size="sm" className={cn("w-10 h-10 shadow-sm hover:shadow-md transition-all duration-300", currentPage === page && "bg-gradient-to-r from-blue-600 to-purple-600 border-0")} onClick={() => setCurrentPage(page)}>
                      {page}
                    </Button>;
            })}
              </div>
              <Button variant="outline" size="sm" disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="px-4 shadow-sm hover:shadow-md transition-all duration-300">
        ...
        <div className="mt-8 p-8 bg-gradient-to-r from-blue-50/80 via-purple-50/80 to-blue-50/80 rounded-2xl border-2 border-slate-200/50 shadow-lg backdrop-blur-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="outline" size="sm" onClick={() => setEditingSummary(!editingSummary)} className="gap-2">
              {editingSummary ? <Save className="w-4 h-4" /> : <Edit3 className="w-4 h-4" />}
              {editingSummary ? 'Save' : 'Edit'}
            </Button>
          </div>
          
          {editingSummary ? <div className="space-y-4">
              <Textarea value={customSummary} onChange={e => setCustomSummary(e.target.value)} placeholder="Enter your custom summary for this table..." className="min-h-32 bg-white" />
              <div className="flex gap-2">
            ...
            </div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
